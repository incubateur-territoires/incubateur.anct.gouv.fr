  ...
  <Layout>
    <div class="recrutement">
      ...
      <div class="job-cards">
        ...
        <g-link to="/recrutements/" class="button">
          Retourner aux offres en cours →
        </g-link>
      </div>
    </div>
  </Layout>
...
<page-query>
query {
  jobs: allJob (sortBy: "poste_ouvert") {
    edges {
      node {
        id
        path
...
export default {
  ...
  computed: {
    jobs() {
      return this.$page.jobs.edges.filter(e => this.$date().isAfter(e.node.poste_ferme))
    }
  }
}
</script>
